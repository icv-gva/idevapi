...
// Helper seguro para parsear arrays de parámetros de alias (prefijo/sufijo/tipo)
// Soporta tanto comillas simples ['a','b'] como comillas dobles ["a","b"]
function _parseParamArray(str) {
	try {
		return JSON.parse(str.replace(/'/g, '"'));
	} catch (e) {
		console.warn('IDEVAPI: Error parseando parámetro de alias: ' + str, e);
		return ['', ''];
	}
}
...
function añadeCapaGeoJSON(contador, mapa, idCapa, data, capa, tipoArbol, servicios, capasGeoJSON, capasNormales, datosTabla, controlCapaActual) {
			...
			for (var prop in capa.tablaInfo.alias) {
				...
				if (prop.split("|").length > 1) {
					...
					if (IDEVAPI_global.idioma == "es") {
						prefijo = _parseParamArray(paramsExtra.split(";")[0])[0];
						sufijo = _parseParamArray(paramsExtra.split(";")[1])[0];
						tipo = _parseParamArray(paramsExtra.split(";")[2])[0];
					} else {
						prefijo = _parseParamArray(paramsExtra.split(";")[0])[1];
						sufijo = _parseParamArray(paramsExtra.split(";")[1])[1];
						tipo = _parseParamArray(paramsExtra.split(";")[2])[1];
					}
	...
	if (!data || !data.features || data.features.length == 0) {
		console.warn("IDEVAPI: La capa GeoJSON '" + capa.titulo + "' no contiene elementos o los datos son inválidos.");
		var tipoGeometria = "Point"; // Valor por defecto para evitar errores posteriores si la capa está vacía
	} else {
	...
	if (tipoGeometria == "Point") {
		...
		geoJsonLayer.datos = capa.servicio.datos;
		...
		geoJsonLayer.ordenLeyenda = contador;
		////////////////////////////////////////// Capa Cluster /////////////////////////////////////////////////
		if (capa.cluster.activo) {
			...
			$('head').append('<style type="text/css">.idevapi-marker-micluster-' + idCSS + ' {background-color: ' + colorClusterE + ';border-radius: ' + borderRadiusE + 'px;} .idevapi-marker-micluster-' + idCSS + ' div {width:' + anchoAltoInt + 'px ; height:' + anchoAltoInt + 'px; margin-left:' + marginInt + 'px ; margin-top:' + marginInt + 'px; border-radius:' + borderRadius + 'px ;background-color: ' + colorClusterI + ';color: ' + capa.cluster.colorTxt + ';} .idevapi-marker-micluster-' + idCSS + ' div span{line-height:' + lineHeightTxt + 'px; font-family:ArialNarrowBold; font-size:' + fontSize + 'px;}</style>');
			var markerCluster = L.markerClusterGroup.layerSupport({
				...
				iconCreateFunction: function (cluster) {	//Función que crea el icono del cluster
					...
					return new L.DivIcon({ html: '<div id="' + idCapa + '"><span>' + childCount + '</span></div>', className: 'marker-cluster idevapi-marker-micluster-' + idCSS, iconSize: new L.Point(tamaño, tamaño) });
				}
			});
			markerCluster.idInterno = capa.idInterno;
			geoJsonLayer.markerCluster = markerCluster;
		...
		//Si hay TOC anidado
		geoJsonLayer.TOCDesplegado = capa.TOCDesplegado;
		if (capa.TOCNivel1 !== null) {
			if (capa.TOCNivel2 !== null) {
				capasAHojas(tipoArbol, servicios, capa.TOCNivel2, capa.TOCNivel1, geoJsonLayer, capa.titulo, contador, controlCapaActual.GCapasTree, controlCapaActual.capasOpacidad, controlCapaActual.tocKeySinNivel2);
			} else {
				capasAHojas(tipoArbol, servicios, capa.TOCNivel1, null, geoJsonLayer, capa.titulo, contador, controlCapaActual.GCapasTree, controlCapaActual.capasOpacidad, controlCapaActual.tocKeySinNivel2);
			}
			//Si el TOC es NO anidado
		} else {
			if (capa.TOCVisible) {
				if (controlCapaActual && typeof controlCapaActual.registraCapaSueltaTOC === 'function') {
					controlCapaActual.registraCapaSueltaTOC(tipoArbol, capa.titulo, geoJsonLayer, contador);
				} else {
...
function actualizaCapaIntervalo2(capa) {
	...
	if (capa.origen == "AGS") {
		...
		var clausWhere = capa.filtroSQL;	//"municipio LIKE 'AGOST' AND nombre LIKE 'POLIDEPORTIVO%'";
		var peticionPG = $.ajax({
		...
		if (capa.url == "local" || capa.url == "IDEVAPI_Local") {
			var dataLocal;
			if (typeof capa.datos === 'object' && capa.datos !== null) {
				dataLocal = capa.datos;
			} else if (window[capa.datos] !== undefined) {
				dataLocal = window[capa.datos];
			}

			if (dataLocal !== undefined) {
				//Actualiza el resultado del filtro de la capa
				// Nota: se eliminan comprobaciones de variables indefinidas para evitar errores
				capa = actualizaFiltroCapaGeoJSON(capa, dataLocal);
			} else {
...
function toggleClearIcon(input) {
	var icon = input.parentElement.querySelector('.idevapi-input-clear-icon');
	if (icon == null) {
		var icon = input.parentElement.querySelector('.idevapi-input-clear-icon-fecha');
	}
	if (input.value.trim() !== '') {
		icon.style.visibility = 'visible';
	} else {
		let allEmpty = [...input.parentElement.querySelectorAll('.idevapi-input-filtro-txt, .idevapi-input-filtro-fecha-txt')].every(el => el.value.trim() === '');
...
function actualizaFiltroCambio(capa, evt) {
	$("#idevapi-cargandoMapa_" + capa._map.id).show();
...
function preparaFiltrosCapas(mapa, capa) {
	...
	*/
	...
	function limpiarClicFecha(evt) {
		...
		let inputs = parent.querySelectorAll('.idevapi-input-filtro-fecha-txt');
		inputs.forEach(function (input) {
			input.value = '';
		});
		icon.style.visibility = 'hidden';
	...
	capa.filtro.inputs.forEach(function (inp) {
		...
		if (inp.tipo === 'fecha') {
			...
			var imgLimpiar = $(divInput).find(".idevapi-input-clear-icon-fecha");
			var divRefresh = $(divInput).find(".idevapi-input-refresh-filtro");
			...
			var imgLimpiar = $(divInput).find(".idevapi-input-clear-icon");
			var divRefresh = $(divInput).find(".idevapi-input-refresh-filtro");
	...
	capa.filtro.inputs.forEach(function (inp) {
		if (inp.tipo == 'fecha') {
			if (inp.valorDefectoInicio !== undefined || inp.valorDefectoFin !== undefined) {
				...
				var refreshClic = $(inpDOM).find(".idevapi-input-refresh-filtro:eq(0)");
		...
		estadisticas.forEach(function (estadistica) {
			...
			if (element.length > 0) {
				switch (true) {
					...
					case /desvTipicaFiltrados\{(.+)\}/.test(estadistica.formula):
						const fieldDesvTipicaVisible = /desvTipicaFiltrados\{(.+)\}/.exec(estadistica.formula)[1];
